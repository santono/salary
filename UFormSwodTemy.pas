...
var
  FormSwodTemy: TFormSwodTemy;

implementation
 Uses ScrUtil,ScrDEf,ScrIo,UFormView, UFormSelPKG,ScrLists,dateUtils;
...
procedure TFormSwodTemy.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
      Action:=caFree;
end;

PROCEDURE TFormSwodTemy.MAKE_TEMY_L_F;
...
BEGIN
     ...
     IF FileExists(CDIR+'LABEL.TXT') THEN
        ...
        END;
     writeln(DEV,' ');
     writeln(DEV,'           —¬ŒƒÕ¿ﬂ ¬≈ƒŒÃŒ—“‹ œŒ “≈Ã¿Ã');
   //  writeln(DEV,'                «¿ ',GetMonthRus(NMES));
   //  writeln(DEV,'                «¿ œ≈–»Œƒ — '+GetMonthUkr(MonthOf(DateFr))+' '+IntToStr(YearOf(DateFr))+' ÔÓ '+GetMonthUkr(MonthOf(DateTo))+' '+IntToStr(YearOf(DateTo)));
     if Mode=1 then
        begin
              writeln(DEV,'                ¬ œ≈–»Œƒ≈ — '+GetMonthUkr(MonthOf(DateFr))+' '+IntToStr(YearOf(DateFr))+' ÔÓ '+GetMonthUkr(MonthOf(DateTo))+' '+IntToStr(YearOf(DateTo)));
              writeln(DEV,'                «¿ '+GetMonthUkr(WantedMonth));
        end
     else
        begin
             writeln(DEV,'                «¿ œ≈–»Œƒ — '+GetMonthUkr(MonthOf(DateFr))+' '+IntToStr(YearOf(DateFr))+' ÔÓ '+GetMonthUkr(MonthOf(DateTo))+' '+IntToStr(YearOf(DateTo)));
     ...
     writeln(DEV,'---------------------------------------------------------------------------------------------------------------');
     writeln(DEV,':ÿËÙ:Õ‡ËÏÂÌÓ‚‡ÌËÂ ÔÓ‰‡Á‰ÂÎÂÌËˇ   : “ ≈ Ã ¿ :  ŒÒÌÓ‚Ì‡ˇ  : CÓ‚ÏÂ˘ÂÌËÂ :  » “ Œ √ Œ : ¡ÓÎ¸ÌË˜Ì.5 : ¡ÓÎ¸ÌË˜Ì˚Â :');
     writeln(DEV,'---------------------------------------------------------------------------------------------------------------');
END;
...
PROCEDURE PRINT_ALL_COUNT(VAR OSN_SUMMA,SOWM_SUMMA,ITOGO_SUMMA,
 ...
 PROCEDURE PRINT_COUNT(P:PCOUNT);
  ...
  BEGIN
        ...
        WRITELN(DEV,': » “ Œ √ Œ   œ Œ     ¿ ‘ ≈ ƒ – ≈            :',T,':',TT,':',TTT,':');
     ...
     KZ:=0;
{
     ...
     REWRITE(DEV_082);
     ProgressBar.Min:=1;
     ProgressBar.Max:=COUNT_SERV*iCount;
     ProgressBar.Position:=0;
     Application.ProcessMessages;
     nmes:=MonthOf(DateFr);
     for iCurr:=1 to iCount do
         begin
              if (iCurr>1) then
                 begin
                      nmes:=nmes+1;
                      if NMES>12 then nmes:=1;
                 end;
...
procedure TFormSwodTemy.BitBtn1Click(Sender: TObject);
begin
    Application.CreateForm(TFormSelPKG, FormSelPKG);
    FormSelPKG.ShowModal;
end;

procedure TFormSwodTemy.BitBtn2Click(Sender: TObject);
begin
{
     if NameServList.CountSelected<=0 then
        begin
             ShowMessage('ÕÂ ‚˚·‡Ì˚ ÔÓ‰‡Á‰ÂÎÂÌËˇ');
             Exit;
        end;
     if KategList.CountSelected<=0 then
        begin
             ShowMessage('ÕÂ ‚˚·‡Ì˚ Í‡ÚÂ„ÓËË');
             Exit;
        end;
}        
     if GruppyList.CountSelected<=0 then
        begin
             ShowMessage('ÕÂ ‚˚·‡Ì˚ Ò˜ÂÚ‡');
             Exit;
        end;
     DateFr := dtFrom.Date;
     DateTo := dtTo.Date;
     if (DateFr>DateTo) then
        begin
             ShowMessage('ÕÂ ‚ÂÌÓ ÛÍ‡Á‡Ì˚ ‰‡Ú˚');
             Exit;
        end;
     if ((YearOf(DateTo)-YearOf(DateTo))>2) then
        begin
             ShowMessage('ÕÂ ‚ÂÌÓ ÛÍ‡Á‡Ì˚ ‰‡Ú˚');
             Exit;
        end;
     if (CurrYear-YearOf(DateTo)>2) then
        begin
             ShowMessage('ÕÂ ‚ÂÌÓ ÛÍ‡Á‡Ì˚ ‰‡Ú˚');
             Exit;
        end;
...
procedure TFormSwodTemy.FormCreate(Sender: TObject);
begin
