  ...
  begin
    ...
    begin
      ...
      try
        case FieldType of
          ...
          'D' : begin
                  ...
                  M:=StrToInt(Copy(W, 3, 2)); D:=StrToInt(Copy(W, 1, 2));
      ...
      FModified:=True; // Буфер dbf-файла обновлен
              ...
              'Ряд: '+IntToStr(FCurrentRecord)+', Поле: '+IntToStr(FieldNo)+' Данные='+Data+' W='+W);
  end;
end;
...
begin
  ...
  begin             // Перекодировка из формата ANSI в формат OEM
    while Pos('_', S)>0 do S[Pos('_', S)]:='I';  // Для корректной работы с украинской
    while Pos('_', S)>0 do S[Pos('_', S)]:='i';  // буквой 'i' (замена на латинскую)
