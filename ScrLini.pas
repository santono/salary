 ...
 BEGIN
        ...
        WHILE NOT EOF(DEV) DO
                BEGIN
                    ...
                    if (Upper_String(AllTrim(Short_Name))='ВСЕГО УДЕРЖАНО') then M_Shifr:=Shifr_Shi;
                    New(ShifrRec);
...
//                    ShifrRec^.Add        := IsAdd;
                    ...
                    if Pos('ВСЕГО',Upper_String(AllTrim(Short_NAme)))=1 then
          ...
          begin
              ...
              if IoResult<>0 then
                 begin
                    ...
                    Short_Name:=DosToWin(Short_Name);
                    Long_Name:=DosToWin(Long_Name);
                    if (Upper_String(DosToWin(AllTrim(Short_Name)))='ВСЕГО НАЧИСЛЕНО') then L_Shifr:=Shifr_Shi;
                    New(ShifrRec);
                    ShifrRec^.Shifr      := Shifr_Shi;
                    ShifrRec^.Short_Name := Short_Name;
                    ShifrRec^.Long_Name  := Long_Name;
   ...
   BEGIN
        ...
        WHILE NOT EOF(DEV) DO
                BEGIN
                    ...
                    if S='ВСЕГО НАЧИСЛЕНО' then
                       begin
                             L_Shifr:=Shifr_Shi;
                             Total_Add_Shifr:=Shifr_Shi;
                       end;
                    if S='ВСЕГО УДЕРЖАНО' then
                       begin
                            M_Shifr:=Shifr_Shi;
                            Total_Ud_Shifr:=Shifr_Shi;
                       end;
