...
type
  TFormReMakeSZNMinus = class(TForm)
    ...
    procedure BtnUpdPodohClick(Sender: TObject);
    procedure FormCreate(Sender: TObject);
    procedure BitBtn4Click(Sender: TObject);
    procedure BitBtn5Click(Sender: TObject);
    procedure IzmPeriodPodNalog;
  private
    ...
    procedure MoveVyslPersonPodoh(Curr_Person:Person_Ptr;var NeedPut:boolean);
    function  ReplacePodohPeriod:integer;
...
var
  FormReMakeSZNMinus: TFormReMakeSZNMinus;
...
procedure TFormReMakeSZNMinus.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
     Action:=caFree;
end;

procedure TFormReMakeSZNMinus.BitBtn2Click(Sender: TObject);
begin
      Application.CreateForm(TFormSelPKG,FormSelPKG);
      FormSelPKG.ShowModal;
end;

procedure TFormReMakeSZNMinus.BtnUpdPodohClick(Sender: TObject);
 var RetVal:Integer;
begin
     IzmPeriodPodNalog;
     Exit;
...
procedure TFormReMakeSZNMinus.FormCreate(Sender: TObject);
begin
     Label1.Caption        := '';
     Label2.Caption        := '';
end;

function TFormReMakeSZNMinus.MoveNadb:integer;
...
procedure TFormReMakeSZNMinus.MoveSZPerson(Curr_Person:Person_Ptr;var NeedPut:boolean;WantedShifr:integer);
  var Curr_Cn:CN_PTR;
      Curr_Add:ADD_PTR;
      New_Cn:CN_PTR;
      New_Add:ADD_PTR;
...
function TFormReMakeSZNMinus.FindPersonFor(Curr_Person:Person_Ptr):Person_Ptr;
...
procedure TFormReMakeSZNMinus.MoveVyslPerson(Curr_Person:Person_Ptr;var NeedPut:boolean);
  var Curr_Cn:CN_PTR;
      Curr_Add:ADD_PTR;
      Curr_Ud:UD_PTR;
      New_Cn:CN_PTR;
  ...
  begin
       ...
       SummaNal:=R10(R10(SummaAdd)-R10(SummaUd));
       if Abs(SummaNal)>0.01 then
          begin
               MAKE_UD(Curr_Ud,Curr_Person);
               Curr_Ud.SHIFR  := PODOH_SHIFR;
               Curr_Ud.PERIOD := 3;
               Curr_Ud.YEAR   := 2014-1990;
               Curr_Ud.Summa  := SummaNal;
...
function TFormReMakeSZNMinus.MoveNadbVysl:integer;
 var I_Podr,Cnt:Integer;
     NSRVTmp,NMESTmp:Integer;
     RetVal :Integer;
     NeedPut:Boolean;
...
procedure TFormReMakeSZNMinus.BitBtn4Click(Sender: TObject);
 var RetVal : Integer;
     WantedShifr:Integer;
begin
...
procedure TFormReMakeSZNMinus.BitBtn5Click(Sender: TObject);
 var I_Podr,Cnt:Integer;
     NSRVTmp,NMESTmp:Integer;
     RetVal :Integer;
     NeedPut:Boolean;
...
function TFormReMakeSZNMinus.ReplacePodohPeriod:integer;
 var I_Podr,Cnt:Integer;
     NSRVTmp,NMESTmp:Integer;
     RetVal :Integer;
     NeedPut:Boolean;
...
begin
         ...
         begin
              ...
              while (Curr_Person<>Nil) do
                 begin
                      ...
                      if get_dol_code(Curr_Person)=1520 then
            //          if Curr_Person^.automatic=automatic_mode then
                         MoveVyslPersonPodoh(Curr_Person,NeedPut);
                      Application.ProcessMessages;
                      Curr_Person:=Curr_Person.Next;
                 end;
              i_c_2:=Count_Person;
     ...
     MKFLNM;
     GetInf(True);
     ReplacePodohPeriod:=RetVal;
end;
procedure TFormReMakeSZNMinus.MoveVyslPersonPodoh(Curr_Person:Person_Ptr;var NeedPut:boolean);
  var Curr_Cn:CN_PTR;
      Curr_Add:ADD_PTR;
      Curr_Ud:UD_PTR;
      New_Cn:CN_PTR;
...
procedure TFormReMakeSZNMinus.IzmPeriodPodNalog;
 var RetVal : Integer;
     WantedShifr:Integer;
begin
     if not ((NMES=6) and (WORK_YEAR_VAL=2014)) then
     ...
     WantedShifr := 0;
     RetVal:=ReplacePodohPeriod;
     if RetVal>0 then
        ShowMessage('Изменено '+IntToStr(RetVal)+' надбавок.');
     Close;
