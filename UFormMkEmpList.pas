...
//      FormMovRecalc0416.ShowModal;
      Application.CreateForm(TFormMoveDoplTo156,FormMoveDoplTo156);
      FormMoveDoplTo156.showModal;
end;
...
//              FormMoveAwans2017_12.ShowModal;
              if not ((NMES=10) and (CurrentYear=2021)) then
                 begin
                      ShowMessage('Перенести перерасчет можно только в октябре 2021');
                 end;
...
begin
     if not ((NMES=3) and (CURRYEAR=2023)) then
        begin
             ShowMessage('Рассчитать премию можно только в марте 2023 г.');
             Exit;
        end
     else
        try
