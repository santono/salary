...
type
  ...
  private
    { Private declarations }
     ...
     procedure ExecuteMovePrem10_2021;
...
begin
     if not ((nmes=10) and (CURRYEAR=2021)) then
        begin
             ShowMessage('Перенос возможен только в октябре 2021 г.');
             exit;
        end;
     ...
     if MessageDlg('Выполнить перенос премии в октябре 2021 г.?',
        mtConfirmation, [mbYes, mbNo], 0) = mrYes  then
        begin
//             ExecuteMovePrem10_2018;
//             ExecuteMovePrem12_2019;
//             ExecuteMovePrem10_2020;
             ExecuteMovePrem10_2021;
...
procedure TFormMovePremFromSQL.ExecuteMovePrem12_2019;
const wantedShifr=46;
      id=37;
      wantedPeriod=12;
      wantedYear=2019;
     ...
     begin
         ...
         while (Curr_Person<>NIl) do
           begin
                ...
                SQLStmnt:='select first 1 npp,summa_40 from  tb_prem_2019 where tabno='+intToSTr(Curr_Person^.tabno)+' and (trim(coalesce(guid_bud,''''))='''+GUIDString+''' or trim(coalesce(guid_vne,''''))='''+GUIDString+''')  and coalesce(moved,0)=0';
                ...
                inc(amntofmoved);
                maked:=true;
                sqlStmnt:='update tb_prem_2019 set moved=1 where npp='+intToStr(npp);
                SQLExecute(sqlStmnt);
                Curr_Person:=Curr_Person^.Next;
           end;
          fillPersons:=retVal;
...
procedure TFormMovePremFromSQL.ExecuteMovePrem10_2021;
const wantedShifr=46;
      id=37;
      wantedPeriod=10;
      wantedYear=2021;
     ...
     begin
         ...
         while (Curr_Person<>NIl) do
           begin
                ...
                if curr_person^.tabno=1356 then
                   curr_person^.TABNO:=1356;
                SQLStmnt:='select first 1 npp,summa from  tb_prem_2021 where tabno='+intToSTr(Curr_Person^.tabno)+' and (trim(coalesce(guid_bud,''''))='''+GUIDString+''' or trim(coalesce(guid_vne,''''))='''+GUIDString+''')  and coalesce(moved,0)=0 and npp>0 and tabno>0';
                v:=SQLQueryRecValues(SQLSTmnt);
                if not VarIsArray(v) then
                   begin
                        curr_person:=curr_person^.NEXT;
